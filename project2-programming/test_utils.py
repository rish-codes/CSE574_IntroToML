...
def run_and_testbackward(test_params, expected, max_points):
    ...
    others_file_path = script_dir / 'Reference_Outputs' / test_params['other']
    others = np.load(others_file_path)
    ...
    expected_file_path = script_dir / 'Reference_Outputs' / expected['expected']
    expected_vals = np.load(expected_file_path)
...
def run_and_testforward(test_params, expected, max_points):
    ...
    # Assuming 'Reference_Outputs' is at the same level as the script directory
    script_dir = Path(__file__).parent
    expected_file_path = script_dir / 'Reference_Outputs' / expected['expected']
    expected_vals = np.load(expected_file_path)
...
def run_and_testprediction(test_params, expected, max_points):
    ...
    script_dir = Path(__file__).parent
    params_file_path = script_dir / 'Reference_Outputs' / expected['expected_params']
    ...
    if not init_rand:
        ...
        expected_train_labels_file_path = script_dir / 'Reference_Outputs' / expected["expected_y_hat_train"]
        expected_train_labels_list = load_labels(expected_train_labels_file_path)
        ...
        expected_val_labels_file_path = script_dir / 'Reference_Outputs' / expected["expected_y_hat_val"]
        expected_val_labels_list = load_labels(expected_val_labels_file_path)
