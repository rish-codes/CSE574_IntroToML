...
def one_hot_encoder(y, num_classes):
    ...
    :param y: vector label to be encoded.
    :param num_classes: Number of classes

    :return: One-hot encoded vector representing the label.
    """
    input_matrix = np.zeros(num_classes)
    input_matrix[y] = 1
    return input_matrix
...
def sigmoidBackward(curr, grad_curr):
    ...
    # Computing the gradients for previous layer
...
def NNBackward(x, y, alpha, beta, z, y_hat):
    ...
    y_encoded = one_hot_encoder(y, 10).reshape(-1, 1)
    
    # Gradient of Cross Entropy Loss w.r.t. y_hat
    g_y_hat = softmaxBackward(y_encoded, y_hat)
...
def prediction(tr_x, tr_y, valid_x, valid_y, tr_alpha, tr_beta):
    ...
    train_error = train_error / tr_x.shape[0]
    ...
    valid_error = valid_error / valid_x.shape[0]
